...
const themes = [
  "blue", "dream2", "dream", "fiery", "rainbow", "pastel", "cristal",
  "red", "aqua", "pink", "retro", "sunlight", "teen", "summer", "flower",
  "ghost", "hacker"
];
...
// ✅ اسم المكتبة: AYMAN-FCA | بوت KIRA
module.exports = (text, type) => {
  const s = String(type || "info").toLowerCase();

  if (s === "warn") {
    process.stderr.write(co(`\r[ KIRA-WARN ] > ${text}`) + "\n");
    return;
  }
  if (s === "error") {
    process.stderr.write(chalk.bold.hex("#ff0000")(`\r[ KIRA-ERROR ]`) + ` > ${text}\n`);
    return;
  }
  if (s === "info") {
    process.stderr.write(chalk.bold(co(`\r[ AYMAN-FCA ] > ${text}`)) + "\n");
    return;
  }
