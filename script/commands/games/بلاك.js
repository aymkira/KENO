...
function drawCard(ctx, x, y, card, hidden = false) {
  ...
  if (hidden) {
    ...
    ctx.globalAlpha = 0.2; ctx.strokeStyle = "#4a90d9"; ctx.lineWidth = 1;
  ...
  ctx.save();
  ctx.translate(x + CW - 6, y + CH - 5); ctx.rotate(Math.PI);
  ctx.font = "bold 14px monospace"; ctx.fillStyle = color;
  ctx.textAlign = "left"; ctx.textBaseline = "top";
  ctx.fillText(card.value, 0, 0);
  ctx.font = "13px monospace"; ctx.fillText(card.suit, 0, 15);
  ctx.restore();
...
function drawCorners(ctx, W, H, color) {
  ...
  [[W-50,18,W-18,18,W-18,50],[18,H-50,18,H-18,50,H-18],
   [18,50,18,18,50,18],[W-50,H-18,W-18,H-18,W-18,H-50]]
...
function drawTable(game, phase) {
  ...
  bg.addColorStop(0, "#0d2e1a"); bg.addColorStop(0.6, "#071a0f"); bg.addColorStop(1, "#030d06");
  ...
  for (let i = 0; i < 40; i++) {
    ctx.save(); ctx.globalAlpha = rand()*0.3+0.05;
    ctx.shadowColor = "#4ade80"; ctx.shadowBlur = 8; ctx.fillStyle = "#4ade80";
  ...
  const dealerTotal = phase === "playing" ? cardVal(game.dealer[0]) : handTotal(game.dealer);
  ctx.save();
  ...
  game.dealer.forEach((card, i) => drawCard(ctx, dStartX + i*40, 90, card, phase === "playing" && i > 0));

  ctx.save();
  const divG = ctx.createLinearGradient(50, 0, W-50, 0);
  divG.addColorStop(0, "transparent"); divG.addColorStop(0.3, "#4ade8040");
  divG.addColorStop(0.7, "#4ade8040"); divG.addColorStop(1, "transparent");
  ...
  game.player.forEach((card, i) => drawCard(ctx, pStartX + i*40, 244, card, false));
  ...
  ctx.shadowColor = "#f5c842"; ctx.shadowBlur = 15; ctx.textBaseline = "bottom";
  ...
  ctx.shadowColor = "#4ade80"; ctx.shadowBlur = 15; ctx.textBaseline = "bottom";
  ...
  if (phase === "result") {
    ...
    if (pT > 21)       { resultText = "💀 تجاوزت 21 — خسرت!"; resultColor = "#ef4444"; profit = -game.bet; }
    else if (dT > 21)  { resultText = "🏆 الكازينو تجاوز — فزت!"; resultColor = "#22c55e"; profit = game.bet; }
    else if (pT > dT)  { resultText = "🏆 فزت!"; resultColor = "#22c55e"; profit = game.bet; }
    else if (pT < dT)  { resultText = "💀 خسرت!"; resultColor = "#ef4444"; profit = -game.bet; }
    else               { resultText = "🤝 تعادل!"; resultColor = "#f5c842"; profit = 0; }
    ...
    ctx.shadowBlur = 20; ctx.textBaseline = "bottom";
  ...
  ctx.fillText("KIRA BLACKJACK v2.0", W-28, H-12);
  ctx.restore();
...
async function calcFinal(game, userID) {
  const db  = getDB();
  if (!db) return;
  const pT  = handTotal(game.player);
  const dT  = handTotal(game.dealer);
  ...
  if (pT > 21 || (pT < dT && dT <= 21)) {
    // خسر
    const r = await db.removeMoney(userID, bet);
    game.money = r.balance ?? game.money - bet;
  } else if (dT > 21 || pT > dT) {
    // فاز
    const newBal = await db.addMoney(userID, bet);
    game.money   = newBal;
  }
  // تعادل — ما يتغير
}
...
module.exports.run = async function({ api, event }) {
  ...
  if (cmd === "سحب") {
    ...
    if (!game) return api.sendMessage("❌ ما عندك لعبة! اكتب: بلاك [رهان]", threadID, messageID);
    ...
    if (total > 21) {
      if (db) { const r = await db.removeMoney(senderID, game.bet); game.money = r.balance ?? game.money - game.bet; }
      games.delete(senderID);
      return sendImage(api, game, "result", threadID, messageID);
    }
    if (total === 21) {
      dealerPlay(game);
      await calcFinal(game, senderID);
      games.delete(senderID);
      return sendImage(api, game, "result", threadID, messageID);
    }
  ...
  if (cmd === "وقف") {
    const game = games.get(senderID);
    if (!game) return api.sendMessage("❌ ما عندك لعبة!", threadID, messageID);
    dealerPlay(game);
    await calcFinal(game, senderID);
    games.delete(senderID);
    return sendImage(api, game, "result", threadID, messageID);
  ...
  if (handTotal(game.player) === 21) {
    ...
    const newBal  = await db.addMoney(senderID, bonus);
    game.money    = newBal;
    return sendImage(api, game, "result", threadID, messageID);
  }
  ...
  // انتهاء المهلة — خسر الرهان
  setTimeout(async () => {
    if (games.has(senderID)) {
      games.delete(senderID);
      if (db) await db.removeMoney(senderID, bet).catch(() => {});
    }
  }, 3 * 60 * 1000);
