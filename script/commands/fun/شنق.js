...
module.exports.config = {
  ...
  hasPermssion: 0,
  credits: "ايمن",
  description: "شنق شخص ما (يدعم المنشن، الأيدي، والرد)",
  commandCategory: "fun",
  usages: "شنق [@منشن/ID/رد]",
  cooldowns: 5,
  dependencies: {
    "axios": "",
    "fs-extra": "",
...
module.exports.handleEvent = async function({ api, event }) {
  ...
  // حذف الرسالة عند التفاعل بـ 😡 من قبل أي مستخدم
...
module.exports.run = async function({ api, event, args, Users, Threads, Currencies, models }) {
  ...
  if (!targetID) {
    return api.sendMessage("⌬ ━━ 𝗞𝗜𝗥𝗔 FUN ━━ ⌬\n\nعليك عمل منشن، أو رد، أو كتابة الأيدي لمن تريد شنقه", threadID, messageID);
  }
  ...
  const backgroundPath = path.join(cacheDir, "hang_bg.png");
  ...
  try {
    ...
    // جلب الخلفية (قالب المشنقة)
    if (!fs.existsSync(backgroundPath)) {
      const bgRes = await axios.get("https://i.postimg.cc/brq6rDDB/received-1417994055426496.jpg", { responseType: "arraybuffer" });
    ...
    const circleOne = await jimp.read(await circle(avatarOnePath)); // الشخص الذي ينفذ الشنق
    const circleTwo = await jimp.read(await circle(avatarTwoPath)); // الشخص المشنوق

    // ضبط الإحداثيات لتناسب صورة المشنقة
    // تم إخفاء صورة الشانق بعيداً أو تصغيرها وتركيز الصورة على المشنوق حسب القالب
    baseImage.composite(circleTwo.resize(118, 118), 350, 80);

    const buffer = await baseImage.getBufferAsync("image/png");
    fs.writeFileSync(pathImg, buffer);

    return api.sendMessage({
      body: `⌬ ━━ 𝗞𝗜𝗥𝗔 FUN ━━ ⌬\n\nتم تنفيذ حكم الإعدام بحق ${targetName} بواسطة ${senderName} ☠️`,
    ...
    if (fs.existsSync(pathImg)) fs.unlinkSync(pathImg);
    return api.sendMessage(`⌬ ━━ 𝗞𝗜𝗥𝗔 FUN ━━ ⌬\n\nحدث خطأ في تنفيذ العملية!`, threadID, messageID);
  }
};
