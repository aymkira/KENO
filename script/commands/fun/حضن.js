...
module.exports.config = {
  ...
  hasPermssion: 0,
  credits: "ايمن",
  description: "احتضن شخصاً ما (حذف بالتفاعل لأي مستخدم)",
  commandCategory: "fun",
  usages: "حضن [@منشن/ID/رد]",
  cooldowns: 5,
  dependencies: {
    "axios": "",
    "fs-extra": "",
...
module.exports.handleEvent = async function({ api, event }) {
  ...
  // التحقق: إذا كان التفاعل 👍 والرسالة التي تم التفاعل عليها هي رسالة البوت
  // تمت إزالة شرط مطابقة senderID ليعمل الحذف للجميع
...
module.exports.run = async function({ api, event, args, Users, Threads, Currencies, models }) {
  ...
  if (!targetID) {
    return api.sendMessage("⌬ ━━ 𝗞𝗜𝗥𝗔 FUN ━━ ⌬\n\nعليك عمل منشن، أو رد، أو كتابة الأيدي لمن تريد احتضانه", threadID, messageID);
  }

  const pathImg = path.join(cacheDir, `hug_${senderID}_${targetID}.png`);
  const avatarOnePath = path.join(cacheDir, `avt_${senderID}.png`);
  const avatarTwoPath = path.join(cacheDir, `avt_${targetID}.png`);
  const backgroundPath = path.join(cacheDir, "hug_bg.png");
  ...
  try {
    ...
    if (!fs.existsSync(backgroundPath)) {
      const bgRes = await axios.get("https://i.ibb.co/3YN3T1r/q1y28eqblsr21.jpg", { responseType: "arraybuffer" });
    ...
    if (fs.existsSync(pathImg)) fs.unlinkSync(pathImg);
    return api.sendMessage(`⌬ ━━ 𝗞𝗜𝗥𝗔 FUN ━━ ⌬\n\nعذراً، حدث خطأ في معالجة الصورة.`, threadID, messageID);
  }
};
